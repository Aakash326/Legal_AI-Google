...
export default function AnalysisPage() {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: `Legal Analysis - ${analysis.document_title || 'Document'}`,
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">
                  {analysis.document_title || 'Legal Document Analysis'}
                </h1>
                <div className="flex items-center gap-4 text-sm text-gray-500 mt-1">
                  <span>Analyzed {formatTimeAgo(analysis.created_at || new Date().toISOString())}</span>
                  {analysis.file_size && (
                    <span>{formatFileSize(analysis.file_size)}</span>
                  )}
                  <Badge variant="outline">
                    {analysis.enhancement_enabled ? 'Enhanced Analysis' : 'Basic Analysis'}
                  </Badge>
                </div>
              </div>
            </div>
            ...
            {analysis.summary && (
              <div className="bg-white rounded-lg p-4 border border-gray-200 max-w-4xl">
                <h2 className="font-medium text-gray-900 mb-2">Executive Summary</h2>
